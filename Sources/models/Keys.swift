		...
		switch key {
			...
			case .S:
				...
				case .t:
					...
					await MapBox.updateTile(newTile: .init(type: .npc(tile: .init(type: .builder, positionToWalkTo: .init(x: p.x, y: p.y - 50, mapType: .mainMap), tilePosition: .init(x: p.x, y: p.y, mapType: .mainMap, oldTile: .init(type: .cactus, biome: .plains)))), event: .talkToNPC, biome: .plains))
