enum MinerNPC {
	static func talk() async {
		...
		if await Game.shared.stages.blacksmith.stage1Stages == .goToMine {
			...
			await Game.shared.stages.blacksmith.stage1Stages = .bringItBack
			await Game.shared.stages.blacksmith.stage1AIronUUIDsToRemove = await Game.shared.player.collect(item: .init(type: .iron, canBeSold: false), count: 5)
			...
			await Game.shared.stages.blacksmith.stage4Stages = .bringItBack
			await Game.shared.stages.blacksmith.stage4CoalUUIDsToRemove = await Game.shared.player.collect(item: .init(type: .coal, canBeSold: false), count: 5)
			...
			await Game.shared.stages.blacksmith.stage8MaterialsToRemove = uuids1 + uuids2
			await Game.shared.stages.blacksmith.stage8Stages = .makeSteel
			...
			await Game.shared.stages.builder.stage1ItemsUUIDsToRemove = uuids1 + uuids2
			await Game.shared.stages.builder.stage1Stages = .bringBack
		} else {
			await getStage()
		}
	}
	...
	static func stage1() async {
		switch await Game.shared.stages.mine.stage1Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage1Stages = .collect
			case .collect:
			...
			case .bringBack:
				...
				await Game.shared.stages.mine.stage1Stages = .done
	...
	static func stage2() async {
		switch await Game.shared.stages.mine.stage2Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage2PickaxeUUIDToRemove = await Game.shared.player.collect(item: .init(type: .pickaxe(type: .init()), canBeSold: false))
				await Game.shared.stages.mine.stage2Stages = .mine
			case .mine:
	...
	static func stage3() async {
		switch await Game.shared.stages.mine.stage3Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage3AxeUUIDToRemove = await Game.shared.player.collect(item: .init(type: .axe(type: .init()), canBeSold: false))
				await StatusBox.quest(.mine3)
				await Game.shared.stages.mine.stage3Stages = .collect
			case .collect:
	...
	static func stage4() async {
		switch await Game.shared.stages.mine.stage4Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage4Stages = .collectPickaxe
			case .collectPickaxe:
				await MessageBox.message("It doesn't look like you got a pickaxe. from the \("Blacksmith".styled(with: .bold)). His building it marked with an \("!".styled(with: [.bold, .red]))", speaker: .miner)
			case .mine:
				if await Game.shared.player.has(item: .stone, count: 30) {
					...
					await Game.shared.stages.mine.stage4Stages = .done
					await Game.shared.player.stats.miningSkillLevel = .three
	...
	static func stage5() async {
		switch await Game.shared.stages.mine.stage5Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage5Stages = .mine
				await Game.shared.stages.mine.stage5PickaxeUUIDToRemove = await Game.shared.player.collect(item: .init(type: .pickaxe(type: .init()), canBeSold: false))
			case .mine:
	...
	static func stage6() async {
		switch await Game.shared.stages.mine.stage6Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage6Stages = .goGetAxe
			case .goGetAxe:
			...
			case .collect:
				if await Game.shared.player.has(item: .lumber, count: 100) {
					...
					if await !Game.shared.player.hasPickaxe() {
						await MessageBox.message("Uh oh, looks like you lost your pickaxe, here is a new one.", speaker: .miner)
						await Game.shared.stages.mine.stage6AxeUUIDToRemove = await Game.shared.player.collect(item: .init(type: .axe(type: .init()), canBeSold: false))
					}
	...
	static func stage7() async {
		switch await Game.shared.stages.mine.stage7Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage7ItemUUIDsToRemove = uuids1 + uuids2 + uuids3
				await Game.shared.stages.mine.stage7Stages = .upgrade
			case .upgrade:
	...
	static func stage8() async {
		switch await Game.shared.stages.mine.stage8Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage8Stages = .getPickaxe
			case .getPickaxe:
	...
	static func stage9() async {
		switch await Game.shared.stages.mine.stage9Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage9Stages = .mine
			case .mine:
				if await Game.shared.player.has(item: .gold, count: 5) {
					...
					await Game.shared.stages.mine.stage9Stages = .done
					await Game.shared.player.stats.miningSkillLevel = .nine
					...
					if await !Game.shared.player.hasPickaxe() {
						await MessageBox.message("Uh oh, looks like you lost your pickaxe, here is a new one.", speaker: .miner)
						await Game.shared.stages.mine.stage9PickaxeUUIDToRemove = await Game.shared.player.collect(item: .init(type: .pickaxe(type: .init()), canBeSold: false))
					}
	...
	static func stage10() async {
		switch await Game.shared.stages.mine.stage10Stages {
			case .notStarted:
				...
				await Game.shared.stages.mine.stage10Stages = .goToSalesman
				await Game.shared.stages.mine.stage10GoldUUIDsToRemove = await Game.shared.player.collect(item: .init(type: .gold, canBeSold: false), count: 5)
			case .goToSalesman:
