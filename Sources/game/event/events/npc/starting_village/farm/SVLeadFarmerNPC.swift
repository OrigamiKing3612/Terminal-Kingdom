enum SVLeadFarmerNPC: StartingVillageNPC {
	...
	static func getStage() async {
		switch await Game.shared.stages.farm.stageNumber {
			case 0:
				...
				let selectedOption = await MessageBox.messageWithOptions("Hello \(Game.shared.player.name)! Would you like to learn how to be a farmer?", speaker: .lead_farmer, options: options)
	...
	static func stage1() async {
		switch await Game.shared.stages.farm.stage1Stages {
			case .notStarted:
				await MessageBox.message("We need some seeds to plant. Can you go chop down a tree and get a tree seed?", speaker: .lead_farmer)
			...
			case .collect:
				if await Game.shared.player.has(item: .tree_seed) {
					...
					if let id = await Game.shared.stages.farm.stage1AxeUUIDToRemove, await !Game.shared.player.has(id: id) {
						await MessageBox.message("Uh oh, looks like you lost your axe. Here is a new one.", speaker: .lead_farmer)
						await Game.shared.stages.farm.setStage1AxeUUIDsToRemove(Game.shared.player.collect(item: .init(type: .axe(type: .init(durability: 1)), canBeSold: false)))
					}
					await MessageBox.message("You are almost there, but you still need to get a tree seed.", speaker: .lead_farmer)
				}
			case .done:
				await Game.shared.stages.farm.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_farmer) {
					await getStage()
				}
		}
	}
	...
	static func stage2() async {
		switch await Game.shared.stages.farm.stage2Stages {
			case .notStarted:
				await MessageBox.message("Now that we have seeds, we need to plant them. There is a pot in the room under me, go plant it in there.", speaker: .lead_farmer)
			...
			case .plant:
				await MessageBox.message("You haven't planted the seed yet.", speaker: .lead_farmer)
			case .comeback:
	...
	static func stage3() async {
		switch await Game.shared.stages.farm.stage3Stages {
			case .notStarted:
				await MessageBox.message("Now we need to wait for the tree to grow. After it has grown, collect it and bring it to me.", speaker: .lead_farmer)
				await Game.shared.stages.farm.setStage3Stages(.collect)
				await StatusBox.quest(.farm3)
			case .collect:
				await MessageBox.message("You haven't collected the tree yet.", speaker: .lead_farmer)
			case .comeBack:
				await MessageBox.message("Great job!", speaker: .lead_farmer)
	...
	static func stage4() async {
		switch await Game.shared.stages.farm.stage4Stages {
			case .notStarted:
				await MessageBox.message("Now I want to teach you how to get a pot to plant your own stuff. Can you go get 10 clay from the mine?", speaker: .lead_farmer)
			...
			case .collect:
				if await Game.shared.stages.farm.stage4Stages.canGetClay {
					await MessageBox.message("You haven't collected the clay from the mine yet.", speaker: .lead_farmer)
				} else {
					await MessageBox.message("You haven't collected the clay from the miner yet.", speaker: .lead_farmer)
				}
			case .comeBack:
				if await Game.shared.player.has(item: .clay, count: 10) {
					...
					if await Game.shared.stages.farm.stage4Stages.canGetClay {
						await MessageBox.message("You don't have the clay from the mine.", speaker: .lead_farmer)
					} else {
						await MessageBox.message("You don't have the clay from the miner.", speaker: .lead_farmer)
					}
				}
			case .done:
				await Game.shared.stages.farm.next()
	...
	static func stage5() async {
		switch await Game.shared.stages.farm.stage5Stages {
			case .notStarted:
				await MessageBox.message("Now that you have the clay, you can take it to the potter and get 5 pots.", speaker: .lead_farmer)
			...
			case .collect:
				await MessageBox.message("You haven't gotten the pot yet.", speaker: .lead_farmer)
			case .comeBack:
