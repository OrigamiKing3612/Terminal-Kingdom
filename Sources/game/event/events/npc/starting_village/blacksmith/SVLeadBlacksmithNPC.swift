enum SVLeadBlacksmithNPC: StartingVillageNPC {
	static func talk() async {
		await NPC.setTalkedTo()
		if await Game.shared.stages.mine.stage1Stages == .collect {
			await MessageBox.message("Ah, here you are. Here is your pickaxe.", speaker: .lead_blacksmith)
			...
			await MessageBox.message("Here you are. Here is your pickaxe.", speaker: .lead_blacksmith)
			...
			await MessageBox.message("Here you are. Here is your axe.", speaker: .lead_blacksmith)
			...
			await MessageBox.message("Here you are. Here is your gift.", speaker: .lead_blacksmith)
	...
	static func getStage() async {
		switch await Game.shared.stages.blacksmith.stageNumber {
			case 0:
				...
				let selectedOption = await MessageBox.messageWithOptions("Hello \(Game.shared.player.name)! Would you like to learn how to be a blacksmith?", speaker: .lead_blacksmith, options: options)
	...
	static func stage1() async {
		switch await Game.shared.stages.blacksmith.stage1Stages {
			case .notStarted, .goToMine:
				...
				await MessageBox.message("I need you to go get some iron from the mine. Then bring it back to me. The door to the mine will be a \"\("!".styled(with: [.red, .bold]))\" to help you find your way.", speaker: .lead_blacksmith)
				await StatusBox.quest(.blacksmith1)
			case .bringItBack:
				if await Game.shared.player.has(item: .iron, count: 5) {
					await MessageBox.message("Thank you!", speaker: .lead_blacksmith)
			...
			case .done:
				await Game.shared.stages.blacksmith.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_blacksmith) {
					await getStage()
				}
		}
	}
	...
	static func stage2() async {
		switch await Game.shared.stages.blacksmith.stage2Stages {
			case .notStarted:
				...
				await MessageBox.message("Now I need you to get 20 lumber. Here is an axe.", speaker: .lead_blacksmith)
	...
	static func stage3() async {
		switch await Game.shared.stages.blacksmith.stage3Stages {
			case .notStarted:
				await MessageBox.message("Now I need you to give this lumber to the carpenter to get sticks.", speaker: .lead_blacksmith)
			...
			case .goToCarpenter:
				await MessageBox.message("You haven't gone to the carpenter yet.", speaker: .lead_blacksmith)
			case .comeBack:
	...
	static func stage4() async {
		switch await Game.shared.stages.blacksmith.stage4Stages {
			case .notStarted:
				await MessageBox.message("I need you to get 5 coal from the miner. We need the iron, lumber and this coal, because I want to show you how to make a pickaxe.", speaker: .lead_blacksmith)
				await Game.shared.stages.blacksmith.setStage4Stages(.collect)
				await StatusBox.quest(.blacksmith4)
			case .collect:
				await MessageBox.message("You haven't gotten the coal yet.", speaker: .lead_blacksmith)
			case .bringItBack:
	...
	static func stage5() async {
		switch await Game.shared.stages.blacksmith.stage5Stages {
			case .notStarted:
				await MessageBox.message("Now you get to do the fun stuff. I need to you make some steel. Go over to the furnace (\(StationTileType.furnace(progress: .empty).render))", speaker: .lead_blacksmith)
			...
			case .makeSteel:
				await MessageBox.message("You haven't gone to the furnace yet. It is labeled with an \"\(StationTileType.furnace(progress: .empty).render)\"", speaker: .lead_blacksmith)
	...
	static func stage6() async {
		switch await Game.shared.stages.blacksmith.stage6Stages {
			case .notStarted:
				await MessageBox.message("I need you to make a pickaxe. Go over to the anvil (\(StationTileType.anvil.render)) and make a pickaxe. Here is all of the things you will need.", speaker: .lead_blacksmith)
			...
			case .makePickaxe:
				await MessageBox.message("You haven't gone to the anvil yet. It is labeled with an \"\(StationTileType.anvil.render)\"", speaker: .lead_blacksmith)
			case .returnToBlacksmith:
				if await Game.shared.player.hasPickaxe() {
					await MessageBox.message("Yay! You made your first Pickaxe!", speaker: .lead_blacksmith)
					...
					await MessageBox.message("Somehow, you haven't made the pickaxe yet.", speaker: .lead_blacksmith)
				}
			case .done:
				await Game.shared.stages.blacksmith.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_blacksmith) {
	...
	static func stage7() async {
		switch await Game.shared.stages.blacksmith.stage7Stages {
			case .notStarted:
				await MessageBox.message("The hunter asked me to make him a sword. Why don't you do that? Here is the stuff you need. Make a sword on the anvil and then bring it to the Hunter in the \(DoorTileTypes.hunting_area.name.styled(with: .bold)).", speaker: .lead_blacksmith)
			...
			case .makeSword:
				await MessageBox.message("You haven't gone to the anvil yet. It is labeled with an \"\(StationTileType.anvil.render)\"", speaker: .lead_blacksmith)
			case .bringToHunter:
				await MessageBox.message("You haven't brought the sword to the hunter yet. The \(DoorTileTypes.hunting_area.name.styled(with: .bold)) is marked with an \("!".styled(with: [.bold, .red])).", speaker: .lead_blacksmith)
			case .comeBack:
	...
	static func stage8() async {
		switch await Game.shared.stages.blacksmith.stage8Stages {
			case .notStarted:
				await MessageBox.message("You are almost there to becoming a blacksmith! I need you to get some materials from the mine. Then I need you to make some steel. Then come back to me", speaker: .lead_blacksmith)
				await Game.shared.stages.blacksmith.setStage8Stages(.getMaterials)
				await StatusBox.quest(.blacksmith8)
			case .getMaterials:
				await MessageBox.message("You haven't gotten the materials yet.", speaker: .lead_blacksmith)
			case .makeSteel:
				await MessageBox.message("You haven't made the steel at the furnace yet.", speaker: .lead_blacksmith)
			case .comeBack:
	...
	static func stage9() async {
		switch await Game.shared.stages.blacksmith.stage9Stages {
			case .notStarted:
				await MessageBox.message("Now I want you to sell this steel in the shop. The shop will be marked with an \"\("!".styled(with: [.bold, .red]))\"", speaker: .lead_blacksmith)
			...
			case .goToSalesman:
				await MessageBox.message("You haven't gone to the salesman yet.", speaker: .lead_blacksmith)
			case .comeBack:
