enum SVLeadMinerNPC: StartingVillageNPC {
	static func talk() async {
		await NPC.setTalkedTo()
		if await Game.shared.stages.blacksmith.stage1Stages == .goToMine {
			await MessageBox.message("Ah, here you are. This is the iron the \("Blacksmith".styled(with: .bold)) needs.", speaker: .lead_miner)
			...
			await MessageBox.message("Yes let me get that for you.", speaker: .lead_miner)
			...
			if await Game.shared.stages.farm.stage4Stages.canGetClay {
				await MessageBox.message("You know how to do it, I'll let you practice this time. Take this pickaxe and go get it from the mine.", speaker: .lead_miner)
	...
	static func getStage() async {
		if await Game.shared.stages.mine.isDone {
			await MessageBox.message("Thank you for helping me. You have completed all of the tasks I have for you.", speaker: .lead_miner)
		} else {
			switch await Game.shared.stages.mine.stageNumber {
				case 0:
					if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .no {
						...
						let selectedOption = await MessageBox.messageWithOptions("Hello \(Game.shared.player.name)! Would you like to learn how to mine?", speaker: .lead_miner, options: options)
	...
	static func stage0() async {
		if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber != .yes {
			if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .no {
				await MessageBox.message("Before I can teach you how to mine, you need to learn how to chop lumber.", speaker: .lead_miner)
			}
			await RandomEventStuff.teachToChopLumber(by: .lead_miner)
			if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .yes {
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
					await stage1()
				}
			}
		} else {
			await MessageBox.message("Hello \(Game.shared.player.name)! Looks like you already know how to chop lumber.", speaker: .lead_miner)
			let options: [MessageOption] = [
				.init(label: "Yes", action: {}),
				.init(label: "No", action: {}),
			]
			let selectedOption = await MessageBox.messageWithOptions("Would you like to learn how to mine?", speaker: .lead_miner, options: options)
			if selectedOption.label == "Yes" {
				await stage1()
			}
		}
	...
	static func stage1() async {
		switch await Game.shared.stages.mine.stage1Stages {
			case .notStarted:
				await MessageBox.message("To mine, you need a pickaxe. Go get one from the \("Blacksmith".styled(with: .bold)), he will have one for you.", speaker: .lead_miner)
				await StatusBox.quest(.mine1)
				await Game.shared.stages.mine.setStage1Stages(.collect)
			case .collect:
				await MessageBox.message("It doesn't look like you got a pickaxe. from the \("Blacksmith".styled(with: .bold)). His building it marked with an \("!".styled(with: [.bold, .red]))", speaker: .lead_miner)
			case .bringBack:
				await MessageBox.message("Thank you for getting the pickaxe!", speaker: .lead_miner)
	...
	static func stage2() async {
		switch await Game.shared.stages.mine.stage2Stages {
			case .notStarted:
				...
				await MessageBox.message("I need you to mine 20 clay. You can do this by mining. The mine entrance the 'M' above and below me. is Here is a pickaxe. Be careful, it can only be used 50 times. ", speaker: .lead_miner)
				await StatusBox.quest(.mine2)
				// TODO: make a door in the mine to come back
				await MessageBox.message("Oh also, press '\(KeyboardKeys.L.render)' to leave the mine.", speaker: .lead_miner)
			...
			case .mine:
				if await Game.shared.player.has(item: .clay, count: 20) {
					...
					if let stage2AxeUUIDToRemove = await Game.shared.stages.mine.stage2PickaxeUUIDToRemove, await !Game.shared.player.has(id: stage2AxeUUIDToRemove) {
						await MessageBox.message("Uh oh, looks like you lost your pickaxe, here is a new one.", speaker: .lead_miner)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .clay) - 20)) clay.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage3() async {
		switch await Game.shared.stages.mine.stage3Stages {
			case .notStarted:
				await MessageBox.message("We need 50 lumber to upgrade the mine to be able to mine more stuff. Can you please go get 50 lumber and bring it back to me?", speaker: .lead_miner)
			...
			case .collect:
				if await Game.shared.player.has(item: .lumber, count: 50) {
					await MessageBox.message("Thank you for getting the lumber! Now we can upgrade the mine.", speaker: .lead_miner)
					...
					if let id = await Game.shared.stages.mine.stage3AxeUUIDToRemove, await !Game.shared.player.has(id: id) {
						await MessageBox.message("Uh oh, looks like you lost your axe, here is a new one.", speaker: .lead_miner)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .lumber) - 50)) lumber.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage4() async {
		switch await Game.shared.stages.mine.stage4Stages {
			case .notStarted:
				await MessageBox.message("Now that we have upgraded the mine, you can go to level 2! There you can find things like stone, coal, and iron.", speaker: .lead_miner)
				await MessageBox.message("I need you to get a pickaxe from the \("Blacksmith".styled(with: .bold)). Then go get 30 stone for me.", speaker: .lead_miner)
			...
			case .mine:
				if await Game.shared.player.has(item: .stone, count: 30) {
					await MessageBox.message("Thank you for getting the stone!", speaker: .lead_miner)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .stone) - 30)) stone from level 2 of the mine.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage5() async {
		switch await Game.shared.stages.mine.stage5Stages {
			case .notStarted:
				await MessageBox.message("Here is another pickaxe. I need you to go get 20 iron for me.", speaker: .lead_miner)
			...
			case .mine:
				if await Game.shared.player.has(item: .iron, count: 20) {
					await MessageBox.message("Thank you for getting the iron!", speaker: .lead_miner)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .iron) - 20)) iron.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage6() async {
		switch await Game.shared.stages.mine.stage6Stages {
			case .notStarted:
				await MessageBox.message("I haven't told you why you need to get all of this stuff yet. We are going to upgrade the mine again. Every time the items required to do so increase. So this time we need 100 lumber to upgrade. You are almost there to being a professional lead_miner!", speaker: .lead_miner)
				await MessageBox.message("Oh, also, I don't have an axe for you. The \("Blacksmith".styled(with: .bold)) can give you one.", speaker: .lead_miner)
				await StatusBox.quest(.mine6)
				await Game.shared.stages.mine.setStage6Stages(.goGetAxe)
			case .goGetAxe:
				await MessageBox.message("I don't have an axe for you. The \("Blacksmith".styled(with: .bold)) can give you one.", speaker: .lead_miner)
			case .collect:
				if await Game.shared.player.has(item: .lumber, count: 100) {
					await MessageBox.message("Thank you for getting the lumber!", speaker: .lead_miner)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .lumber) - 100)) lumber.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage7() async {
		switch await Game.shared.stages.mine.stage7Stages {
			case .notStarted:
				await MessageBox.message("Ok, now that we have the suppilies, we can upgrade the mine. This time I will let you do it! All you have to do is have the materials and go up to the door. There will be an option to upgrade, do that! Then come back to me.", speaker: .lead_miner)
			...
			case .upgrade:
				await MessageBox.message("You haven't upgraded the mine yet. You need to walk up to the building's door and select upgrade.", speaker: .lead_miner)
			case .upgraded:
	...
	static func stage8() async {
		switch await Game.shared.stages.mine.stage8Stages {
			case .notStarted:
				await MessageBox.message("I asked the blacksmith to make a special gift for you. Go collect it from the blacksmith. Then come talk to me.", speaker: .lead_miner)
	...
	static func stage9() async {
		switch await Game.shared.stages.mine.stage9Stages {
			case .notStarted:
				await MessageBox.message("I want to teach you one more thing, but to do that I need to go get 5 gold from level 3 of the mine", speaker: .lead_miner)
			...
			case .mine:
				if await Game.shared.player.has(item: .gold, count: 5) {
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .gold) - 5)) gold.", speaker: .lead_miner)
				}
			case .done:
				await Game.shared.stages.mine.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_miner) {
	...
	static func stage10() async {
		switch await Game.shared.stages.mine.stage10Stages {
			case .notStarted:
				await MessageBox.message("I have one more thing for you to do. I need you to take this gold to the \("Salesman".styled(with: .bold)) and sell it for coins. Then come back to me.", speaker: .lead_miner)
			...
			case .goToSalesman:
				await MessageBox.message("You haven't sold the gold to the \("Salesman".styled(with: .bold)) yet.", speaker: .lead_miner)
			case .comeBack:
