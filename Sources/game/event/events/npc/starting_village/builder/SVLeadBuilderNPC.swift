enum SVLeadBuilderNPC: StartingVillageNPC {
		...
		switch await Game.shared.stages.builder.stageNumber {
			case 0:
				if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .no {
					...
					let selectedOption = await MessageBox.messageWithOptions("Hello \(Game.shared.player.name)! Would you like to learn how to build?", speaker: .lead_builder, options: options)
	...
	static func stage0() async {
		if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber != .yes {
			if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .no {
				await MessageBox.message("Before I can teach you how to build, you need to learn how to chop lumber.", speaker: .lead_builder)
			}
			await RandomEventStuff.teachToChopLumber(by: .lead_builder)
			if await Game.shared.startingVillageChecks.hasBeenTaughtToChopLumber == .yes {
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
					await stage1()
				}
			}
		} else {
			await MessageBox.message("Hello \(Game.shared.player.name)! Looks like you already know how to chop lumber.", speaker: .lead_builder)
			let options: [MessageOption] = [
				.init(label: "Yes", action: {}),
				.init(label: "No", action: {}),
			]
			let selectedOption = await MessageBox.messageWithOptions("Would you like to learn how to build?", speaker: .lead_builder, options: options)
			if selectedOption.label == "Yes" {
				await stage1()
			}
		}
	...
	static func stage1() async {
		switch await Game.shared.stages.builder.stage1Stages {
			case .notStarted:
				await MessageBox.message("Before we can start building, we need materials. Can you go collect 20 stone and 10 iron from the mine and bring it back to me?", speaker: .lead_builder)
				await Game.shared.stages.builder.setStage1Stages(.collect)
				await StatusBox.quest(.builder1)
			case .collect:
				await MessageBox.message("You haven't gone to the mine yet. It will be marked with an \("!".styled(with: .bold)) on the map.", speaker: .lead_builder)
			case .bringBack:
				if await Game.shared.player.has(item: .stone, count: 20), await Game.shared.player.has(item: .iron, count: 10) {
					await MessageBox.message("Great, You have collected the materials!", speaker: .lead_builder)
					...
					await MessageBox.message("You still need to collect the materials. It will be marked with an \("!".styled(with: .bold)) on the map.", speaker: .lead_builder)
				}
			case .done:
	...
	static func stage2() async {
		switch await Game.shared.stages.builder.stage2Stages {
			case .notStarted:
				await MessageBox.message("Now, we need some lumber. Can you get 20 of it? Here is an axe.", speaker: .lead_builder)
			...
			case .collect:
				if await Game.shared.player.has(item: .lumber, count: 20) {
					await MessageBox.message("Great, You have collected the lumber! Now we can start building.", speaker: .lead_builder)
					...
					if let id = await Game.shared.stages.builder.stage2AxeUUIDToRemove, await !Game.shared.player.has(id: id) {
						await MessageBox.message("Uh oh, looks like you lost your axe. Here is a new one.", speaker: .lead_builder)
					...
					await MessageBox.message("You are almost there, but you still need to get \(abs(Game.shared.player.getCount(of: .lumber) - 20)) lumber.", speaker: .lead_builder)
				}
			case .done:
				await Game.shared.stages.builder.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
	...
	static func stage3() async {
		switch await Game.shared.stages.builder.stage3Stages {
			case .notStarted:
				await MessageBox.message("Now that we have the materials, we can start building. Can you make a door at the workstation. It is marked as a \(StationTileType.workbench.render). Here are the materials you will need.", speaker: .lead_builder)
			...
			case .makeDoor:
				await MessageBox.message("You haven't made the door yet. It is marked with a \(StationTileType.workbench.render).", speaker: .lead_builder)
			case .returnToBuilder:
				if await Game.shared.player.has(item: .door(tile: .init(type: .house)), count: 1) {
					await MessageBox.message("Great, You have made the door!", speaker: .lead_builder)
					...
					await MessageBox.message("You still need to make the door. It is marked with a \(StationTileType.workbench.render).", speaker: .lead_builder)
				}
			case .done:
				await Game.shared.stages.builder.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
	...
	static func stage4() async {
		switch await Game.shared.stages.builder.stage4Stages {
			case .notStarted:
				await MessageBox.message("Now that we have the door, we can start building. Can you go talk to the king and ask him for permission to build a house?", speaker: .lead_builder)
				await Game.shared.stages.builder.setStage4Stages(.talkToKing)
				await StatusBox.quest(.builder4)
			case .talkToKing:
				await MessageBox.message("You haven't talked to the king yet.", speaker: .lead_builder)
			case .comeBack:
	...
	static func stage5() async {
		switch await Game.shared.stages.builder.stage5Stages {
			case .notStarted:
				await MessageBox.message("Now that we have permission, we can start building immediately! Can you begin building the house?", speaker: .lead_builder)
				await instructions()
			...
			case .buildHouse:
				if await Game.shared.stages.builder.stage5HasBuiltHouse {
					...
					await MessageBox.message("You haven't built the house yet.", speaker: .lead_builder)
					await MessageBox.messageWithOptions("Do you want to hear the instructions again?", speaker: .lead_builder, options: [.init(label: "Yes", action: instructions), .init(label: "No", action: {})]).action()
				}
			case .done:
				await Game.shared.stages.builder.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
		...
		func instructions() async {
			await MessageBox.message("This is what you need to do: Go pick an area and press \(KeyboardKeys.b.render). This will put you in \("build mode".styled(with: .bold)). Then press \(KeyboardKeys.enter.render), as long as you have 5 lumber you will build a building tile. Place the buildings next to each other. Then place the door in a small area. If you are unsure, look at the other buildings in this village. If you want to see all the controls press \(KeyboardKeys.questionMark.render) in build mode.", speaker: .lead_builder)
		}
	}
	...
	static func stage6() async {
		switch await Game.shared.stages.builder.stage6Stages {
			case .notStarted:
				await MessageBox.message("Now that we have a house, we need to decorate the interior. Can you collect 30 lumber and bring it back to me?", speaker: .lead_builder)
			...
			case .collect:
				if await Game.shared.player.has(item: .lumber, count: 30) {
					await MessageBox.message("Great! You have collected the lumber! Now we can start decorating the inside.", speaker: .lead_builder)
					...
					await MessageBox.message("You are almost there, you you still need to get \(abs(Game.shared.player.getCount(of: .lumber) - 30)) lumber.", speaker: .lead_builder)
				}
			case .done:
				await Game.shared.stages.builder.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
	...
	static func stage7() async {
		switch await Game.shared.stages.builder.stage7Stages {
			case .notStarted:
				await MessageBox.message("Can you take these decorations and organize them inside of the house? You can decorate it however you want!", speaker: .lead_builder)
			...
			case .buildInside:
				if await Game.shared.stages.builder.stage7HasBuiltInside {
					...
					await MessageBox.message("You haven't decorated the interior of your house yet.", speaker: .lead_builder)
				}
			case .done:
				await Game.shared.stages.builder.next()
				if await RandomEventStuff.wantsToContinue(speaker: .lead_builder) {
